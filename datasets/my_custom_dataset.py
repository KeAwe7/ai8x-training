...
class AlzheimersDataset(Dataset):
    """
    `Alzheimer's dataset <https://www.kaggle.com/datasets/preetpalsingh25/alzheimers-dataset-4-class-of-images>` Dataset.
    ...
    label_to_folder_map = {label: label for label in labels}

    def __init__(self, root_dir, d_type, transform=None,
                 resize_size=(128, 128), augment_data=False):
        self.root_dir = root_dir
        self.data_dir = os.path.join(root_dir, 'Alzheimer_s Dataset', d_type)
    ...
    def __print_download_manual(self):
        ...
        print("Unzip the downloaded file and find 'test' and 'train' folders "
              "and copy them into 'data/Alzheimer_s Dataset'. ")
        print("Make sure that images are in the following directory structure:")
        print("  'data/Alzheimer_s Dataset/train/MildDemented'")
        print("  'data/Alzheimer_s Dataset/train/ModerateDemented'")
        print("  'data/Alzheimer_s Dataset/train/NonDemented'")
        print("  'data/Alzheimer_s Dataset/train/VeryMildDemented'")
        print("  'data/Alzheimer_s Dataset/test/MildDemented'")
        print("  'data/Alzheimer_s Dataset/test/ModerateDemented'")
        print("  'data/Alzheimer_s Dataset/test/NonDemented'")
        print("  'data/Alzheimer_s Dataset/test/VeryMildDemented'")
...
def get_alzheimers_dataset(data, load_train, load_test):
    """
    ...
    if load_train:
        train_dataset = AlzheimersDataset(root_dir=data_dir, d_type='train',
                                          transform=transform, augment_data=True)
    else:
        train_dataset = None

    if load_test:
        test_dataset = AlzheimersDataset(root_dir=data_dir, d_type='test', transform=transform)
    else:
        test_dataset = None
...
datasets = [
    {
        ...
        'loader': get_alzheimers_dataset,
    },
    {
        'name': 'my_custom_dataset',
        'input': (3, 128, 128),
        'output': ('MildDemented', 'ModerateDemented', 'NonDemented', 'VeryMildDemented'),
        'loader': get_alzheimers_dataset,  # Assuming the same loader is used
    },
]